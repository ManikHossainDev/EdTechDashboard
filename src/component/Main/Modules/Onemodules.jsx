...
const Onemodules = () => {
  ...
  const handleContentTextChange = (index, value) => {
    setFormData((prev) => {
      ...
      if (block.type === "text") {
        block.content = typeof block.content === "object" ? { ...block.content, text: value } : { text: value, listItems: [] };
      } else {
        block.content = value;
      }
      return cloned;
  ...
  const addContentBlock = (type) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      const newBlock = { type, order: cloned.learningContent.length, content: type === "text" ? { text: "", listItems: [] } : "" };
  ...
  const handleContentImageUpload = async (index) => {
    ...
    fileInput.onchange = async (e) => {
      ...
      try {
        ...
        setFormData((prev) => {
          const cloned = cloneState(prev);
          cloned.learningContent[index].image = { url: imageUrl, publicId: imageUrl.split("/").pop() };
          return cloned;
        });
      } catch (err) {
  ...
  const removeItem = (taskIndex, itemIndex) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      const removedId = cloned.interactiveTasks[taskIndex].config.items[itemIndex].id;
      cloned.interactiveTasks[taskIndex].config.items.splice(itemIndex, 1);
  ...
  const addCategory = (taskIndex) => {
    setFormData((prev) => {
      ...
      cats.push({ id: `cat-${cats.length + 1}`, name: "", description: "", image: { url: "" } });
  ...
  const removeCategory = (taskIndex, catIndex) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      const removedId = cloned.interactiveTasks[taskIndex].config.categories[catIndex].id;
  ...
  const handleCategoryImageUpload = async (taskIndex, catIndex) => {
    ...
    fileInput.onchange = async (e) => {
      ...
      try {
        ...
        setFormData((prev) => {
          const cloned = cloneState(prev);
          cloned.interactiveTasks[taskIndex].config.categories[catIndex].image = { url: imageUrl, publicId: imageUrl.split("/").pop() };
          return cloned;
        });
      } catch (err) {
        console.error("Image upload error:", err);
  ...
  const formatDataForUpdate = () => {
    ...
    const content = formData.learningContent.map((block) => {
      const contentBlock = {
        type: block.type,
        order: block.order,
        content: block.type === "text" ? (typeof block.content === "object" ? block.content.text : block.content) : block.content || "",
      };
      if (block.type === "image" && block.image?.url) {
        contentBlock.image = { url: block.image.url, publicId: block.image.publicId };
      }
      return contentBlock;
    });

    const tasks = formData.interactiveTasks.map((task) => {
      const base = { type: task.type, title: task.title, description: task.description, instructions: task.instructions, points: task.points };
      if (task.type === "drag-drop" || task.type === "sort-categories") {
        base.items = task.config.items.map((it) => ({ id: it.id, text: it.text, ...(it.name && { name: it.name }) }));
        base.categories = task.config.categories.map((cat) => {
          const category = { id: cat.id, name: cat.name, description: cat.description };
          if (cat.image?.url) category.image = { url: cat.image.url, publicId: cat.image.publicId };
          return category;
        });
    ...
    return {
      ...
      quiz,
      parentTip: { title: formData.parentTip.title, content: formData.parentTip.content },
    };
  };
      ...
      <form onSubmit={handleSubmit}>
        ...
        <section className="mb-8 p-4 border rounded-lg">
          ...
          <div className="mb-4">
            ...
            <input type="text" value={formData.title} onChange={(e) => handleInputChange("title", e.target.value)} className="w-full px-3 py-2 border rounded" />
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2">Slug</label>
            <input type="text" value={formData.slug} onChange={(e) => handleInputChange("slug", e.target.value)} className="w-full px-3 py-2 border rounded" />
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2">Theme</label>
            <input type="text" value={formData.theme} onChange={(e) => handleInputChange("theme", e.target.value)} className="w-full px-3 py-2 border rounded" />
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2">Description</label>
            <textarea value={formData.description} onChange={(e) => handleInputChange("description", e.target.value)} rows="3" className="w-full px-3 py-2 border rounded" />
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2">Status</label>
            <select value={formData.status} onChange={(e) => handleInputChange("status", e.target.value)} className="w-full px-3 py-2 border rounded">
        ...
        <section className="mb-8 p-4 border rounded-lg">
          ...
          {formData.introVideo?.url ? (
            <video controls className="w-full max-w-md h-auto rounded border mb-4">
              <source src={formData.introVideo.url} type="video/mp4" />
            </video>
          ) : (
            <p className="text-gray-500 italic mb-4">No intro video uploaded</p>
          )}
          <button type="button" onClick={handleIntroVideoUpload} disabled={isSubmitting} className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded disabled:opacity-50">
        ...
        <section className="mb-8 p-4 border rounded-lg">
          <div className="flex justify-between items-center mb-4">
            ...
            <button type="button" onClick={addObjective} className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded">Add</button>
          </div>
          {formData.learningObjectives.map((obj, index) => (
            <div key={index} className="mb-3 flex items-center gap-2">
              <span className="text-gray-500 font-medium">{index + 1}.</span>
              <input type="text" value={obj.text} onChange={(e) => handleObjectiveChange(index, e.target.value)} className="flex-1 px-3 py-2 border rounded" placeholder="Enter objective" />
            ...
            <div key={index} className="mb-6 p-4 border rounded">
              ...
              {block.type === "image" && (
                <div className="mb-4 flex items-center gap-4">
                  {block.image?.url ? <img src={block.image.url} alt="Content" className="w-32 h-32 object-cover border rounded" /> : <div className="w-32 h-32 bg-gray-200 border rounded flex items-center justify-center text-gray-500">No image</div>}
              ...
              <textarea value={block.type === "text" ? (typeof block.content === "object" ? block.content.text : block.content) : block.content} onChange={(e) => handleContentTextChange(index, e.target.value)} rows="3" className="w-full px-3 py-2 border rounded" placeholder="Content text" />
            </div>
          ))}
        </section>
          ...
          {formData.interactiveTasks.map((task, taskIndex) => (
            <div key={taskIndex} className="mb-6 p-4 border rounded bg-gray-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="block text-gray-700 text-sm font-bold mb-2">Title</label>
                  <input type="text" value={task.title} onChange={(e) => handleTaskChange(taskIndex, "title", e.target.value)} className="w-full px-3 py-2 border rounded" />
                </div>
                <div>
                  <label className="block text-gray-700 text-sm font-bold mb-2">Points</label>
                  <input type="number" value={task.points} onChange={(e) => handleTaskChange(taskIndex, "points", parseFloat(e.target.value))} className="w-full px-3 py-2 border rounded" />
                </div>
              </div>
              <div className="mb-4">
                <label className="block text-gray-700 text-sm font-bold mb-2">Description</label>
                <textarea value={task.description} onChange={(e) => handleTaskChange(taskIndex, "description", e.target.value)} rows="2" className="w-full px-3 py-2 border rounded" />
              </div>
              <div className="mb-4">
                <label className="block text-gray-700 text-sm font-bold mb-2">Instructions</label>
                <textarea value={task.instructions} onChange={(e) => handleTaskChange(taskIndex, "instructions", e.target.value)} rows="2" className="w-full px-3 py-2 border rounded" />
              </div>

              {(task.type === "drag-drop" || task.type === "sort-categories") && task.config && (
                <>
                  {/* Items */}
                  <div className="mb-6 p-3 border rounded bg-white">
                    ...
                    {task.config.items?.map((item, itemIndex) => (
                      <div key={itemIndex} className="mb-3 p-2 border rounded flex items-center gap-2">
                        <input type="text" value={item.text} onChange={(e) => handleItemChange(taskIndex, itemIndex, "text", e.target.value)} placeholder="Item text" className="flex-1 px-2 py-1 border rounded" />
                        <select value={task.config.correctMapping?.[item.id] || ""} onChange={(e) => handleMappingChange(taskIndex, item.id, e.target.value)} className="w-40 px-2 py-1 border rounded">
                          ...
                          {task.config.categories?.map((cat) => (<option key={cat.id} value={cat.id}>{cat.name}</option>))}
                        </select>
                  ...
                  <div className="p-3 border rounded bg-white">
                    ...
                    {task.config.categories?.map((cat, catIndex) => (
                      <div key={catIndex} className="mb-3 p-2 border rounded flex items-center gap-2">
                        {cat.image?.url ? <img src={cat.image.url} alt={cat.name} className="w-12 h-12 object-cover border rounded" /> : <div className="w-12 h-12 bg-gray-200 border rounded flex items-center justify-center text-xs">No img</div>}
        ...
        <section className="mb-8 p-4 border rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input type="text" value={formData.quiz.title} onChange={(e) => handleNestedChange("quiz", "title", e.target.value)} className="w-full px-3 py-2 border rounded" />
            </div>
            <div>
              <label className="block text-gray-700 text-sm font-bold mb-2">Passing Score (%)</label>
              <input type="number" value={formData.quiz.passingScore} onChange={(e) => handleNestedChange("quiz", "passingScore", parseInt(e.target.value))} className="w-full px-3 py-2 border rounded" />
            </div>
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2">Description</label>
            <textarea value={formData.quiz.description} onChange={(e) => handleNestedChange("quiz", "description", e.target.value)} rows="2" className="w-full px-3 py-2 border rounded" />
          </div>
          <div className="flex gap-4 mb-6">
            <label className="flex items-center gap-2"><input type="checkbox" checked={formData.quiz.allowRetake} onChange={(e) => handleNestedChange("quiz", "allowRetake", e.target.checked)} /> Allow Retake</label>
            <label className="flex items-center gap-2"><input type="checkbox" checked={formData.quiz.showCorrectAnswers} onChange={(e) => handleNestedChange("quiz", "showCorrectAnswers", e.target.checked)} /> Show Correct Answers</label>
          </div>
          ...
          {formData.quiz.questions.map((q, qIndex) => (
            <div key={qIndex} className="mb-6 p-4 border rounded bg-gray-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <label className="block text-gray-700 text-sm font-bold mb-2">Points</label>
                  <input type="number" value={q.points} onChange={(e) => handleQuestionChange(qIndex, "points", parseFloat(e.target.value))} className="w-full px-3 py-2 border rounded" />
                </div>
              </div>
              <div className="mb-4">
                <label className="block text-gray-700 text-sm font-bold mb-2">Question Text</label>
                <textarea value={q.question} onChange={(e) => handleQuestionChange(qIndex, "question", e.target.value)} rows="2" className="w-full px-3 py-2 border rounded" />
              </div>
              <div className="mb-4">
                <label className="block text-gray-700 text-sm font-bold mb-2">Explanation</label>
                <textarea value={q.explanation} onChange={(e) => handleQuestionChange(qIndex, "explanation", e.target.value)} rows="2" className="w-full px-3 py-2 border rounded" />
              </div>
              <div>
                ...
                {q.options.map((opt, optIndex) => (
                  <div key={optIndex} className="mb-2 p-2 border rounded flex items-center gap-2 bg-white">
                    ...
                    <label className="flex items-center gap-1 text-sm"><input type="radio" name={`quiz-correct-${qIndex}`} checked={opt.isCorrect} onChange={() => handleOptionChange(qIndex, optIndex, "isCorrect", true)} /> Correct</label>
        ...
        <section className="mb-8 p-4 border rounded-lg">
          ...
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2">Title</label>
            <input type="text" value={formData.parentTip.title} onChange={(e) => handleNestedChange("parentTip", "title", e.target.value)} className="w-full px-3 py-2 border rounded" />
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2">Content</label>
            <textarea value={formData.parentTip.content} onChange={(e) => handleNestedChange("parentTip", "content", e.target.value)} rows="4" className="w-full px-3 py-2 border rounded" />
          </div>
        </section>

        {/* Submit */}
        <div className="flex justify-end">
          <button type="submit" disabled={isSubmitting} className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-6 rounded disabled:opacity-50">

...
const Onemodules = () => {
  ...
  const removeItem = (taskIndex, itemIndex) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      const removedId =
        cloned.interactiveTasks[taskIndex].config.items[itemIndex].id;
      cloned.interactiveTasks[taskIndex].config.items.splice(itemIndex, 1);
      delete cloned.interactiveTasks[taskIndex].config.correctMapping[
        removedId
      ];
      return cloned;
    });
  ...
  const removeCategory = (taskIndex, catIndex) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      const removedId =
        cloned.interactiveTasks[taskIndex].config.categories[catIndex].id;
      ...
      try {
        ...
        setFormData((prev) => {
          ...
          cloned.interactiveTasks[taskIndex].config.categories[catIndex].image =
            { url: imageUrl, publicId: imageUrl.split("/").pop() };
          return cloned;
        });
      } catch (err) {
        console.error("Image upload error:", err);
  ...
  const handleMappingChange = (taskIndex, itemId, categoryId) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      cloned.interactiveTasks[taskIndex].config.correctMapping[itemId] =
        categoryId;
      return cloned;
    });
  };
  ...
  const formatDataForUpdate = () => {
    ...
    const tasks = formData.interactiveTasks.map((task) => {
      ...
      if (task.type === "drag-drop" || task.type === "sort-categories") {
        base.items = task.config.items.map((it) => ({
          id: it.id,
          text: it.text,
      ...
      <form onSubmit={handleSubmit}>
        ...
        <section className="mb-8 p-4 border rounded-lg">
          ...
          {formData.introVideo?.url ? (
            <video
              controls
              className="w-full max-w-md h-auto rounded border mb-4"
            >
        ...
        <section className="mb-8 p-4 border rounded-lg">
          ...
          {formData.interactiveTasks.map((task, taskIndex) => (
            <div key={taskIndex} className="mb-6 p-4 border rounded bg-gray-50">
              ...
              {(task.type === "drag-drop" || task.type === "sort-categories") &&
                task.config && (
                  <>
                    {/* Items */}
                    <div className="mb-6 p-3 border rounded bg-white">
                      ...
                      {task.config.items?.map((item, itemIndex) => (
                        <div
                          key={itemIndex}
                          className="mb-3 p-2 border rounded flex items-center gap-2"
                        >
                          <input
                            type="text"
                            value={item.text}
                          ...
                          <button
                            type="button"
                            onClick={() => removeItem(taskIndex, itemIndex)}
                      ...
                      {task.config.categories?.map((cat, catIndex) => (
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              handleCategoryImageUpload(taskIndex, catIndex)
                            }

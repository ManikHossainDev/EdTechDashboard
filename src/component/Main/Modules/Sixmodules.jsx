...
const Sixmodules = () => {
  ...
  // Mood Meter states
  const handleMoodStateChange = (taskIndex, stateIndex, field, value) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      cloned.interactiveTasks[taskIndex].config.moodMeter.states[stateIndex][
        field
      ] = value;
      return cloned;
    });
  ...
  const addMoodState = (taskIndex) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      const states =
        cloned.interactiveTasks[taskIndex].config.moodMeter?.states || [];
      const newId = `state${states.length + 1}`;
      states.push({
        id: newId,
        name: "",
        emoji: "",
      ...
      cloned.interactiveTasks[taskIndex].config.moodMeter.states = states;
      return cloned;
    });
  };

  const removeMoodState = (taskIndex, stateIndex) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      cloned.interactiveTasks[taskIndex].config.moodMeter.states.splice(
        stateIndex,
        1,
      );
      cloned.interactiveTasks[taskIndex].config.moodMeter.states.forEach(
        (s, i) => (s.order = i + 1),
      );
      return cloned;
    });
  ...
  const handleMoodMeterChange = (taskIndex, field, value) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      cloned.interactiveTasks[taskIndex].config.moodMeter[field] = value;
      return cloned;
    });
  };

  // Friends
  const handleFriendChange = (taskIndex, friendIndex, field, value) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      cloned.interactiveTasks[taskIndex].config.friends[friendIndex][field] =
        value;
      return cloned;
    });
  };

  const addFriend = (taskIndex) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      const friends = cloned.interactiveTasks[taskIndex].config.friends || [];
      const newId = `friend${friends.length + 1}`;
      friends.push({ id: newId, name: "", emoji: "", icon: "" });
      cloned.interactiveTasks[taskIndex].config.friends = friends;
      return cloned;
    });
  };

  const removeFriend = (taskIndex, friendIndex) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      cloned.interactiveTasks[taskIndex].config.friends.splice(friendIndex, 1);
      return cloned;
    });
  };
  ...
  const addScenario = (taskIndex) => {
    setFormData((prev) => {
      ...
      const newId = `c${scenarios.length + 1}`;
      scenarios.push({
        id: newId,
        text: "",
        situation: "",
        ...
        responses: [
          ...
          { id: "r2", text: "", feedback: "", isCorrect: false },
        ],
      });
  ...
  // Scenario responses (for chat-simulator)
  const handleResponseChange = (
    ...
    setFormData((prev) => {
      const cloned = cloneState(prev);
      cloned.interactiveTasks[taskIndex].config.scenarios[scIndex].responses[
        respIndex
      ][field] = value;
      return cloned;
    });
  ...
  const addResponse = (taskIndex, scIndex) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      const responses =
        cloned.interactiveTasks[taskIndex].config.scenarios[scIndex].responses;
      const newId = `r${responses.length + 1}`;
      responses.push({ id: newId, text: "", feedback: "", isCorrect: false });
      return cloned;
    });
  };

  const removeResponse = (taskIndex, scIndex, respIndex) => {
    setFormData((prev) => {
      const cloned = cloneState(prev);
      cloned.interactiveTasks[taskIndex].config.scenarios[
        scIndex
  ...
  const formatDataForUpdate = () => {
    ...
    const tasks = formData.interactiveTasks.map((task) => {
      ...
      if (task.type === "chat-simulator" && task.config) {
        base.config = {
          scenarios: (task.config.scenarios || []).map((sc) => ({
            ...
            hint: sc.hint || "",
            responses: (sc.responses || []).map((resp) => ({
              id: resp.id,
              text: resp.text,
        ...
        <section className="mb-8 p-4 border rounded-lg">
          ...
          {formData.interactiveTasks.map((task, taskIndex) => (
            <div key={taskIndex} className="mb-6 p-4 border rounded bg-gray-50">
              ...
              {/* chat-simulator specific config */}
              {task.type === "chat-simulator" && task.config && (
                <>
                  {/* Mood Meter */}
                  <div className="mb-6 p-3 border rounded bg-white">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <select
                          value={task.config.moodMeter?.initialState || ""}
                          onChange={(e) =>
                            handleMoodMeterChange(
                              taskIndex,
                              "initialState",
                              e.target.value,
                            )
                      ...
                      <div className="flex items-center">
                        <label className="flex items-center gap-2">
                          <input
                            type="checkbox"
                            checked={task.config.moodMeter?.showMeter ?? true}
                            onChange={(e) =>
                              handleMoodMeterChange(
                                taskIndex,
                                "showMeter",
                    ...
                    {task.config.moodMeter?.states?.map((state, stateIndex) => (
                      <div
                        key={stateIndex}
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => removeMoodState(taskIndex, stateIndex)}
                  ...
                  <div className="mb-6 p-3 border rounded bg-white">
                    ...
                    {task.config.friends?.map((friend, friendIndex) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => removeFriend(taskIndex, friendIndex)}
                          className="bg-red-500 hover:bg-red-600 text-white px-2 py-1 rounded text-sm"
                        >
                          X
                        </button>
                      ...
                      >
                        ...
                        <div className="mb-3">
                          ...
                          <input
                            ...
                            placeholder="e.g., Alex asks for your password"
                          />
                        </div>
                        <div className="mb-3">
                          <label className="block text-gray-700 text-sm font-bold mb-1">
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <button
                              type="button"
                              onClick={() => addResponse(taskIndex, scIndex)}
                              className="bg-green-500 hover:bg-green-600 text-white px-2 py-1 rounded text-xs"
                            >
                              Add Response
                            </button>
                          </div>
                          {scenario.responses?.map((resp, respIndex) => (
                            <div
                              key={respIndex}
